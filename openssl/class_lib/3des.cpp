//编译: g++ -ggdb3 -o 3des_test 3des.cpp -lssl -lcrypto
...
//如果填充3 个char buf[8], 实在太麻烦了. 这里固定一种模式, 由1 to 3.
...
//3des ebc 加密(注意: strlen(key_str),意味着只能传入一串字符串,而且长度不能超出8 字节.)
...
void des_ebc3_encrypt(char* key_str, char* data, char* ret, int buf_len){
	...
	char *key_str2 = (char*)malloc(8);
	char *key_str3 = (char*)malloc(8);
	...
	for(i = 0;i < datalen/8;i++){
		...
		DES_ecb3_encrypt(&data_tmp_in, &data_tmp_out, \
				&schedule, &schedule2, &schedule3, DES_ENCRYPT);
...
void des_ebc3_decrypt(char* key_str, char* data, char* ret, int buf_len){
	...
	for(i = 0;i < datalen/8;i++){
		memcpy(data_tmp_in,data+i*8,8);
		DES_ecb3_encrypt(&data_tmp_in, &data_tmp_out, \
				&schedule, &schedule2, &schedule3, DES_DECRYPT);
		memcpy(ret+i*8,data_tmp_out,8);
	}
...
//3des cbc 加密(char* key_str 长度不限, char* ivec 必须为8 字节,)
void des_cbc3_encrypt(char* key_str, \
	...
	*/
	...
	DES_ede3_cbc_encrypt((unsigned char*)data, (unsigned char*)ret, \
			(long int)datalen, &schedule, &schedule2, &schedule3, \
			&key_ivec, DES_ENCRYPT);
...
//测试函数封装
char *smart_point = NULL;//这个指针的存在, 使得这个测试程序是单线程版本的
...
void des_ebc3_encrypt_test(void){
	...
	//结束打印
	print_text(des_ebc_data);
	print_encrypted(des_ebc_ret);

	//继承后续测试-解密
	smart_point = des_ebc_ret;
}
...
void des_cbc3_encrypt_test(void){
	...
	//执行加密
	des_cbc3_encrypt(des_cbc_key,des_cbc_ivce,\
			des_cbc_data,des_cbc_ret,des_cbc_buf_len);
	//结束打印
	print_text(des_cbc_data);
	print_encrypted(des_cbc_ret);

	//继承后续测试-解密
	smart_point = des_cbc_ret;
}
...
void des_cbc3_decrypt_test(void){
	...
	int des_cbc_buf_len = 512+8;
	//执行加密
	des_cbc3_decrypt(des_cbc_key,des_cbc_ivce,\
			smart_point,des_cbc_ret,des_cbc_buf_len);
	//结束打印
	print_text(des_cbc_ret);
